...
const trimWhiteSpace = (
    ...
    if (newPd !== prodTitle || newEp !== epTitle || newNum !== epNumber) status = 'modified';
...
const moveArticles = (
    ...
    const articles = ["El ", "Il ", "Lo ", "La ", "Los ", "Las ", "L'", "The ", "An ", "A ", "Un ", "Una "];
    let art = null;
    let status: FileItem['status'] = currentStatus;

    art = articles.filter( art => prodTitle.toLowerCase().startsWith(art.toLowerCase()) );
    prodTitle = art.length ? `${prodTitle.substring(art[0].length)}, ${art[0].toUpperCase().trim()}` : prodTitle;    

    status = art.length ? 'modified' : currentStatus;
    
    art = articles.filter( art => epTitle.toLowerCase().startsWith(art.toLowerCase()) );
    epTitle = art.length ? `${epTitle.substring(art[0].length)}, ${art[0].trim()}` : epTitle;
...
const capitalizeAB = (
    ...
    if (matches) {
        ...
        const newNumber = `Ep No. ${result.join(' - ')} ${epNumber.substring(remIndex)}`.trim() 
...
const dotify = (
    ...
    if (endings.includes(epTitle[epTitle.length - 1])) epTitle = epTitle.substring(0, epTitle.length - 1); // don't end on special char
...
const dotifyTitle = (title: string): [string, FileItem['status']] => {
    ...
    [prodTitle, epTitle, epNumber, status] = trimWhiteSpace(prodTitle, epTitle, epNumber, status);
    ...
    if (cleanTitle.length > 60 || epTitle.search(/(\.\.\.)$/) !== -1) {
        ...
        epTitle = `${epTitle.substring(0, epTitle.search(/(\s\.\s\.\s\.)$/))}${epTitle.substring(epTitle.search(/(\s\.\s\.\s\.)$/) + 1)}`;
        cleanTitle = buildFullTitle(prodTitle, epTitle, epNumber);
        status = 'modified';
    }
    return [cleanTitle, status];
